...
| 3f | Off-site entity signals | **3 of 5 done — 2026-09-16; LinkedIn + Instagram left, by hand** | Checked 2026-09-16. Already link back: GitHub (website shivambhadoriya.com, location Navsari — read via `gh api user`), X (shivambhadoriya.com, Navsari), WakaTime (shivambhadoriya.com, Navsari). **LinkedIn:** Website still `shivam-bhadoriya-dev.vercel.app`, location "Greater Ahmedabad Area" — change both by hand. Do not automate it: LinkedIn loads PerimeterX (`uc=scraping`) and hung the CDP session. **Instagram:** personal account, bio has no link; add the site or drop it from `sameAs` (Shivam's call). |
