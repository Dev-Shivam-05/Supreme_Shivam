 ...
 */
export function ServicePageView({ page, proof }: { page: ServicePage; proof: Project[] }) {
  return (
    <>
      <header className="relative overflow-hidden pt-36 md:pt-44">
        ...
        <div className="container-x relative pb-14 md:pb-20">
          ...
          <Reveal delay={0.06}>
            <p className="mt-8 max-w-2xl text-lg leading-relaxed text-fg-muted">{page.lede}</p>
          </Reveal>

          {/* Price and timeline sit above the fold on purpose — clients filter on
              both, and a page that hides them wastes the visit. */}
          <Reveal delay={0.12}>
            <div className="mt-10 grid max-w-3xl gap-px overflow-hidden border border-border bg-border sm:grid-cols-2">
              <div className="bg-bg-elev p-6">
                <div className="hud">Starts at</div>
                <div className="mt-2 font-poster text-4xl text-accent">{page.priceFrom}</div>
                <p className="mt-2 text-sm text-fg-muted">{page.priceNote}</p>
              </div>
              <div className="bg-bg-elev p-6">
                <div className="hud">Typical timeline</div>
                <div className="mt-2 font-display text-xl font-semibold text-fg">{page.timeline}</div>
              ...
              <ul className="space-y-5">
                {page.deliverables.map((d) => (
                  <li key={d.title} className="flex gap-3">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-fg">{d.title}</div>
                      <p className="mt-1 text-sm leading-relaxed text-fg-muted">{d.body}</p>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <section className="border-y border-border bg-bg-elev">
          <div className="container-x py-16 md:py-24">
            ...
            <h2 className="font-poster display-3 max-w-3xl">
              Shipped, measured, <span className="text-accent">inspectable.</span>
            </h2>
            <div className="mt-12 grid gap-px overflow-hidden border border-border bg-border md:grid-cols-3">
              {proof.map((p) => (
                <Link key={p.slug} href={`/work/${p.slug}`} className="group bg-bg p-8">
                  ...
                  <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 border-t border-border pt-5">
                    {p.metrics.slice(0, 2).map((m) => (
                      <div key={m.label}>
                        <div className="font-poster text-lg text-accent">{m.value}</div>
            ...
            {page.faqs.map((f) => (
              <details key={f.q} className="group border-b border-border py-6">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
                  <h3 className="font-display text-lg font-medium text-fg md:text-xl">{f.q}</h3>
                ...
                <p className="mt-4 max-w-2xl leading-relaxed text-fg-muted">{f.a}</p>
              </details>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-end justify-between gap-10">
            <div>
              ...
              <h2 className="font-poster display-2 max-w-3xl leading-[0.85]">
                Tell me the problem.
                <br />
                <span className="text-accent">I&apos;ll tell you the price.</span>
              </h2>
              <p className="mt-8 max-w-xl text-lg text-fg-muted">
                A free 30-minute call, no obligation. If I am not the right person for it, I will
                say so and point you at who is.
              </p>
            </div>
            <Link
              href="/contact"
