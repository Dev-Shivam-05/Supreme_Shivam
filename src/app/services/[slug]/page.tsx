...
import { getServicePage, serviceOnly } from "@/lib/services";
import { site } from "@/lib/site";

type Params = { params: Promise<{ slug: string }> };

export function generateStaticParams() {
  return serviceOnly.map((p) => ({ slug: p.slug.replace("/services/", "") }));
}

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const { slug } = await params;
  const page = getServicePage(`/services/${slug}`);
  if (!page) return {};
...
export default async function ServiceRoute({ params }: Params) {
  ...
  // Keep the order declared in lib/services.ts rather than the project order.
